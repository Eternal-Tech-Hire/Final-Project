...
class Company {
  ...
  static async getByEmail(email: string) {
    return (await db
      .collection("Company")
      .findOne({ email })) as CompanyTypes | null;
  }
  ...
  static async update(updateData: CompanyTypes, _id: string) {
    ...
    const data = (await db
      .collection("Company")
      .findOne({ id })) as CompanyTypes | null;
    if (!data) {
    ...
    return await db.collection("Company").updateOne(
      { _id: id },
      {
        $set: {
          ...
          password:
            updateData.password != null ? updateData.password : data.password,
          jobOffer:
            updateData.jobOffer != null ? updateData.jobOffer : data.jobOffer,
        },
      }
    );
  }
  ...
  static async updateFavEvent(
    ...
    const duplicate = favs.some((item: any) => item.seekerId.equals(idSeeker));
    if (duplicate) {
